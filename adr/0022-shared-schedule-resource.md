...
- References: `docs/architecture/01-os-feature-map.md` features 2, 14; ADR-0008,
  ADR-0017, ADR-0019, ADR-0021; `resolve-control-plane-gaps` D5

## Context

Feature 2 gains scheduled snapshots with retention (resolve-architecture-gaps
D5), and feature 14 needs a scrub cadence (ADR-0021). Both are recurring,
time-based work driven by the reconciler. Without a shared model, each feature
builds its own scheduler and duplicates cadence, retention, and cron-like
expression handling.
...
schedules that reference them and reconcile accordingly. Scope: `Schedule` covers
data/app dataset snapshots and scrubs only. The earlier `config`-dataset snapshot
clause (resolve-control-plane-gaps D13) is dropped — the `config` dataset no
longer exists (ADR-0007) and system config state lives on OS-disk partitions
without fs snapshots (ADR-0011).
