...
> Discovery-phase design. Authored from the `storage-anchor` openspec change and
> extended by `datasets-schedules`.
> The decisions D-S1–D-S11 here fix how the `Disk`, `Pool`, `Dataset`,
...
- The scrub *controller* itself (disk-health, ADR-0021) — the schedule engine
  is shared, but the scrub consumer is a later change; scrub schedules validate
  to `Ready` with `lastResult: skipped`.
- Disk replacement (`POST /v1/disks/{id}/replace`) and scrub *scheduling*
  beyond the imperative scrub action and the validated scrub schedule.
...
- Pool destruction on `DELETE /v1/pools/{id}`: the contract declares it
  ("accepted, deletion in progress"), but the framework's deletion finalizer
  landed with `datasets-schedules` for the content layer; pool destruction is
  wired to the same finalizer path in a later change.
...
ListDevices  DeviceHealth SmartInfo
...
complete for the pool/disk scope) and emitted by a later change with the
replacement flow (ADR-0024).
...
controller). Valid schedules report `Ready` with `nextRun`; scrub schedules
validate to `Ready`/`lastResult: skipped` because the scrub consumer is a
later change (ADR-0021, non-goal).

## 14. Wiring into the daemon (D6)
...
2. construct the five storage controllers — `DiskController(host,
   ...
   and `ScheduleController(registry)` (the schedule controller never shells
   out: the engine is pure and firing goes through the store) — controllers
   receive the metric sink (consumer-defined `MetricSink` interface, satisfied
   by the daemon registry) and never build providers themselves (D8);
...
## 15. Status shapes (contract envelope)
...
- **Disk** `actual`: `device`, `state` (unassigned/spare/in-use/retired/
  ...
  `smartSummary`, `resilverProgress` (reserved).
...
- **Deletion finalizers**: the framework hands a tombstone (`Deleted`) with
  the deleted resource name to the owning controller; Dataset and Snapshot
  controllers destroy host state on removal. Pool destruction reuses the same
  path in a later change. The Snapshot finalizer releases its controller-applied
  hold before `zfs destroy` (a held snapshot refuses destruction) and never
  forces past an external hold — a delete of a snapshot under backup
  in-flight retries until the hold clears. Deletion ordering matters for
  datasets: a dataset whose snapshots carry holds should have those holds
  released (snapshot resources deleted/held cleared) before the Dataset
  resource is deleted, because `zfs destroy -r` refuses held snapshots and the
  holds map is per-controller.
