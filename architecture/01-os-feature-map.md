...
| 8 | Web-UI | Management console over the API; no direct host access. Status: scaffold + resource-screen framework landed; Identity (users, roles, sessions, tokens) and Storage (pools, disks, datasets, zvols, snapshots, schedules) groups live with descriptor-driven screens, contract-generated spec forms, and shared action/reference/structured-widget/typed-status primitives; remaining groups follow — see `docs/architecture/14-web-ui.md` (D-W18, D-W19), ADR-0033 |
...
| Web-UI front door | Static server terminates browser TLS with certificates-controller material; `core` binds loopback; single same-origin entry point for UI + API | [14-web-ui](architecture/14-web-ui.md), [ADR-0033](adr/0033-management-plane-front-door.md), [ADR-0012](adr/0012-web-ui-static-server-in-image.md) |
| Web-UI resource screens | Descriptor registry drives navigation, generated routes, list/detail screens, and operation+capability-gated affordances; contract-generated spec forms and status/action manifests with a full-fidelity YAML/JSON escape hatch; Identity first, Storage (D-W19) live | [14-web-ui](architecture/14-web-ui.md) (D-W18, D-W19) |
